				 ...
				 struct kvm_vcpu *shadow_vcpu)
{
	...
	if (shadow_vcpu->arch.fault.hpfar_el2 >= (0xf0000000 >> 8) &&
		 ...
		 * fail to share the regions if another guest already
		 * has the access.
		 */
...
static void handle___pkvm_iommu_alloc_domain(struct kvm_cpu_context *host_ctxt)
{
...
static void handle___pkvm_iommu_free_domain(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(unsigned int, domain, host_ctxt, 1);

	cpu_reg(host_ctxt, 1) = __pkvm_iommu_free_domain(domain);
}

static void handle___pkvm_iommu_attach_dev(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(unsigned int, iommu, host_ctxt, 1);
	DECLARE_REG(unsigned int, domain, host_ctxt, 2);

	cpu_reg(host_ctxt, 1) = __pkvm_iommu_attach_dev(iommu, domain);
}

static void handle___pkvm_iommu_detach_dev(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(unsigned int, iommu, host_ctxt, 1);
	DECLARE_REG(unsigned int, domain, host_ctxt, 2);

	cpu_reg(host_ctxt, 1) = __pkvm_iommu_detach_dev(iommu, domain);
}

static void handle___pkvm_iommu_map(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(unsigned int, domain, host_ctxt, 1);
...
static void handle___pkvm_iommu_unmap(struct kvm_cpu_context *host_ctxt)
{
	...
	cpu_reg(host_ctxt, 1) = __pkvm_iommu_unmap(domain, iova, size);
}

static void handle___pkvm_iommu_iova_to_phys(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(unsigned int, domain, host_ctxt, 1);
	DECLARE_REG(unsigned long, iova, host_ctxt, 2);
...
handle___pkvm_iommu_flush_iotlb_all(struct kvm_cpu_context *host_ctxt)
{
	...
	cpu_reg(host_ctxt, 1) = __pkvm_iommu_flush_iotlb_all(iommu);
}

static void handle___pkvm_iommu_rk_enable(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(unsigned int, iommu, host_ctxt, 1);

	cpu_reg(host_ctxt, 1) = __pkvm_iommu_rk_enable(iommu);
}

static void handle___pkvm_iommu_rk_disable(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(unsigned int, iommu, host_ctxt, 1);

	cpu_reg(host_ctxt, 1) = __pkvm_iommu_rk_disable(iommu);
}

static void handle___kvm_get_el2_trap_count(struct kvm_cpu_context *host_ctxt)
{
	cpu_reg(host_ctxt, 1) = __kvm_get_el2_trap_count();
}

static void handle___kvm_clear_el2_trap_count(struct kvm_cpu_context *host_ctxt)
{
	__kvm_clear_el2_trap_count();
...
static u64 get_stage2_pt_size(int vm_handle)
{
...
static void handle___pkvm_stage2_pt_count(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(int, vm_handle, host_ctxt, 1);
	cpu_reg(host_ctxt, 1) = get_stage2_pt_size(vm_handle);
}

static void handle___pkvm_get_shadow_handles(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(u32 *, pool_hva, host_ctxt, 1);
	DECLARE_REG(u32, cap, host_ctxt, 2);

	u8 *base = kern_hyp_va((void *)pool_hva);
	size_t bytes = cap * sizeof(u32);
	int ret;
	size_t cnt = 0, i;
	...
	for (i = 0; i < KVM_MAX_PVMS; i++) {
		struct kvm_shadow_vm *vm =
			pkvm_shadow_table_get(index_to_shadow_handle(i));
		if (!vm)
			continue;
		((u32 *)base)[cnt++] = index_to_shadow_handle(i);
	}
	// unpin:
	hyp_unpin_shared_mem(base, base + bytes);
out:
...
static const hcall_t host_hcall[] = {
	...
	HANDLE_FUNC(__pkvm_iommu_alloc_domain),
	HANDLE_FUNC(__pkvm_iommu_free_domain),
	HANDLE_FUNC(__pkvm_iommu_attach_dev),
	HANDLE_FUNC(__pkvm_iommu_detach_dev),
	HANDLE_FUNC(__pkvm_iommu_map),
	HANDLE_FUNC(__pkvm_iommu_unmap),
	HANDLE_FUNC(__pkvm_iommu_iova_to_phys),
	HANDLE_FUNC(__pkvm_iommu_flush_iotlb_all),
	HANDLE_FUNC(__pkvm_iommu_rk_enable),
