			    ...
			    struct kvm_shadow_vm *vm, int vcpu_idx)
{
	...
	/* VFIO device must be attached at guest boot */
	shadow_vcpu->is_device_attached = true;
...
static bool pkvm_reset_rknpu()
{
	...
	/* Reset RKNPU Core 0 */
	writel(0x400040, rockchip_cru_base + 0xa78);
	writel(0x1000100, rockchip_cru_base + 0xa78);
	udelay(10);
	writel(0x400000, rockchip_cru_base + 0xa78);
	writel(0x1000000, rockchip_cru_base + 0xa78);

	/* Reset RKNPU Core 1 */
	writel(0x10001, rockchip_cru_base + 0xa6c);
	writel(0x40004, rockchip_cru_base + 0xa6c);
	udelay(10);
	writel(0x10000, rockchip_cru_base + 0xa6c);
	writel(0x40000, rockchip_cru_base + 0xa6c);

	/* Reset RKNPU Core 2 */
	writel(0x10001, rockchip_cru_base + 0xa70);
	writel(0x40004, rockchip_cru_base + 0xa70);
	udelay(10);
	writel(0x10000, rockchip_cru_base + 0xa70);
	writel(0x40000, rockchip_cru_base + 0xa70);
...
bool kvm_handle_pvm_hvc64(struct kvm_vcpu *vcpu, u64 *exit_code)
{
	...
	case ARM_SMCCC_VENDOR_HYP_KVM_FEATURES_FUNC_ID:
		...
		val[0] |= BIT(ARM_SMCCC_KVM_FUNC_ATTACH_DEVICE);
	...
	case ARM_SMCCC_VENDOR_HYP_KVM_ATTACH_DEVICE_FUNC_ID:
		...
		asm volatile("isb; mrs %0, cntpct_el0" : "=r"(vcpu->time_2));

		/* NOTE: vmid and domain_id could be different.
		 *		 Check if we are passing the correct domain_id.
		 */
		ret = __pkvm_iommu_rk_enable_hyp(vcpu->kvm->arch.mmu.vmid.vmid);
		if (ret) {
			val[0] = SMCCC_RET_INVALID_PARAMETER;
			break;
		}

		vcpu->is_device_attached = true;
		val[0] = SMCCC_RET_SUCCESS;
	...
	case ARM_SMCCC_VENDOR_HYP_KVM_DETACH_DEVICE_FUNC_ID:
		...
		/* Temporarily disable RKNPU reset */
		// pkvm_reset_rknpu();

		vcpu->is_device_attached = false;
		val[0] = SMCCC_RET_SUCCESS;
		asm volatile("isb; mrs %0, cntpct_el0" : "=r"(vcpu->time_3));
		break;
	case ARM_SMCCC_VENDOR_HYP_KVM_GET_HYP_PERF_NUMS_FUNC_ID:
		val[0] = SMCCC_RET_SUCCESS;
		val[1] = vcpu->time_1;
		val[2] = vcpu->time_2;
		val[3] = vcpu->time_3;
