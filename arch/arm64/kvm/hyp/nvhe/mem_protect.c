				      ...
				      void * const arg)
{
	...
	/* HACK: Do not check for PTE validity */
...
static int __hyp_set_page_state_range(u64 addr, u64 size,
				       enum pkvm_page_state state)
{
	enum kvm_pgtable_prot prot = pkvm_mkstate(KVM_PGTABLE_PROT_RWX, state);
...
static int hyp_request_unshare(u64 *completer_addr,
				const struct pkvm_mem_transition *tx)
{
	u64 size = tx->nr_pages * PAGE_SIZE;
	u64 addr = tx->initiator.addr;
...
static int hyp_initiate_unshare(u64 *completer_addr,
				 const struct pkvm_mem_transition *tx)
{
	u64 size = tx->nr_pages * PAGE_SIZE;
	u64 addr = tx->initiator.addr;
	...
	*completer_addr = tx->initiator.hyp.completer_addr;
	return __hyp_set_page_state_range(addr, size, PKVM_PAGE_OWNED);
}
...
static int check_unshare(struct pkvm_mem_share *share)
{
	...
	case PKVM_ID_HOST:
		...
		break;
	case PKVM_ID_HYP:
		ret = hyp_request_unshare(&completer_addr, tx);
		break;
	case PKVM_ID_GUEST:
...
static int __do_unshare(struct pkvm_mem_share *share)
{
	...
	case PKVM_ID_HOST:
		...
		break;
	case PKVM_ID_HYP:
		ret = hyp_initiate_unshare(&completer_addr, tx);
		break;
	case PKVM_ID_GUEST:
